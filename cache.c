...
{
	return cache_create_pdp(name, nsets, bsize, balloc, usize, assoc,
        policy, blk_access_fn, hit_latency,0);
}
/* create and initialize a general cache structure */
struct cache_t *			/* pointer to cache created */
cache_create_pdp(char *name,		/* name of the cache */
	     int nsets,			/* total number of sets in cache */
	     int bsize,			/* block (line) size of cache */
	     int balloc,		/* allocate data space for blocks? */
	     int usize,			/* size of user data to alloc w/blks */
...
cache_char2policy(char c)		/* replacement policy as a char */
{
  ...
  case 'f': return FIFO;
  case 'p': return PDP; 	/* drd: pdp policy */
	     ...
	     int pdp) 			/* drd:PDP on or off */
{
  ...
  case PDP:
	{
	int bindex=0,max_rpd,max_index=0,found_repl=-1;
      for (blk=cp->sets[set].way_head,max_rpd=blk->rpd;
	   blk;
